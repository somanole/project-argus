...
API); scopes `workflow:list` + `project:list` cover the catalog,
`execution:list` (+ `execution:read`) adds **health** (S3), and `user:list` adds the
**advisory inferred owner** (S4 — team-project members/roles + personal-space owners).
If the key lacks the execution scopes, health degrades explicitly to "unavailable" per
workflow (never a false "healthy"); if it lacks `user:list` (or the instance isn't
licensed for project roles), the inferred owner degrades to "couldn't infer" (never a
fabricated name) — explicit assignment is unaffected. In every case the rest of Argus
works. Argus is read-only against every instance.
